...
/** Tests of AdminUserTokens component. */
describe('AdminUserTokens', () => {
  // //////////////////////////////////////////////////////////////////////////
  // No user selected
  ...
  describe('no user selected', () => {
    it('renders filter slot', () => {
      ...
      // Assert: AdminUserTokensFilter is rendered via the filter slot.
      const formFilter = wrapper.find('[data-testid="form-user-filter"]');
      expect(formFilter.exists()).toBe(true);
    });

    it('does not call loadTokensPage on mount', () => {
      // Arrange & Act: Mount with no user.
      createComponent();

      // Assert: No API call made when no user is selected.
      expect(mockLoadTokensPage).not.toHaveBeenCalled();
    });
    ...
    it('shows emptyNoUserText', () => {
      ...
      expect(emptyEl.text()).toBe('No tokens to show.');
    });
  });
  ...
  describe('user selected', () => {
    it('calls loadTokensPage with correct params on mount', async () => {
      ...
      expect(mockLoadTokensPage).toHaveBeenCalledTimes(1);
      const callArg = mockLoadTokensPage.mock.calls[0]?.[0];
    ...
    it('shows emptyText when data is empty with user selected', async () => {
      ...
      expect(tableEmpty.text()).toBe('⚠️ No tokens to show. Check filter settings. ⚠️');
    });
  });
