...
const defaultForm: UserJwtTableFilterForm = {
  userId: -1,
...
function createComponent(form: UserJwtTableFilterForm, isBusy: boolean, disabled?: boolean) {
  return shallowMount(AdminUserJwtFilter, {
    global: {
      plugins: [i18n],
    },
    props: {
...
/** Tests of AdminUserJwtFilter component. */
describe('AdminUserJwtFilter', () => {
  // //////////////////////////////////////////////////////////////////////////
  // Rendering
  ...
  describe('rendering', () => {
    it('renders the filter heading', () => {
      ...
      expect(heading.text()).toBe('🔍 User JWT filter');
    });
  ...
  describe('model binding', () => {
    it('updates createdFromAt when from picker emits', async () => {
      const form: UserJwtTableFilterForm = {
        userId: -1,
        createdFromAt: null,
        createdToAt: null,
        tableMeta: null,
