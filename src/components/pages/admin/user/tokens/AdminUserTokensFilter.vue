...
import type { UserTokenTableFilterForm } from '@/code/data/features/user/admin-user.ts';
  ...
  <div class="form-wide">
    ...
    <form @submit.prevent="emit('reload')" novalidate data-testid="form-user-filter">
      <div class="form-subform">
        <label for="createdFromAt">{{ t('admin.user.tokens.filter.createdFromAt') }}:</label>
        <DateTimePicker
        ...
        <label for="createdToAt">{{ t('admin.user.tokens.filter.createdToAt') }}:</label>
        <DateTimePicker
      ...
      <button type="submit" :disabled="isSubmitting">
        {{ isSubmitting ? t('admin.user.tokens.filter.buttonBusy') : t('admin.user.tokens.filter.button') }}
      </button>
    </form>
  </div>
</template>
